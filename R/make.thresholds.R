make.thresholds <- function(item.params,...) {
	UseMethod("make.thresholds")
}

make.thresholds.character <- function(item.params,design.matrix="normal",...) {
	#print("character")
	return(make.thresholds(CQmodel(show=item.params),...))
}

make.thresholds.CQmodel <- function(item.params,item.table,interactions,step.table,design.matrix = "normal",...) {
